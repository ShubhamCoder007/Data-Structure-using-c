...
/*
N* deleteBalance(N* root, int data){
	if(root == NULL)
		return NULL;
	
	root = delete(root, data);
	
	while(balance(root) >= 2 || balance(root) <= -2){
		printf("balancing...");
		if(balance(root) >= 2){
			N* temp = root;
			while(balance(temp->left) >= 2)
				temp = temp->left;
			temp->left = checkRotate(temp->left);
		}
		if(balance(root) <= -2){
			N* temp = root;
			while(balance(temp->right) <= -2)
				temp = temp->right;
			temp->right = checkRotate(temp->right);
		}
	}
